...
import { macOSAction } from '../../../redux/actions/actions';
import { connect } from 'react-redux';


const PlatformMacOS = ({ gamesMacOS, isMacOSLoaded, getData }) => {
    console.log(gamesMacOS, isMacOSLoaded, getData);
    ...
    return (
        <div id="content" className="main-page">
            ...
            {isMacOSLoaded === true ? <div className="container">

                <div className="gamesDetails">
                    <p>Games for macOS</p>
                    <h1 className="gamesDetails">
                        MacOS (previously OS X) is an operating system used in Apple laptops and computers.
                        Currently, the platform counts as many as 14 significant releases each called correspondingly with wildlife or parks' names.
                        The system is the part of Unix-based families and features layered design as well as enclosed source code which means it is not possible to setup non-Apple PC with Macintosh OS.
                        Usually, all the apps installed in the system must be verified by Apple Store application built-in in OS.
                        Such a system reduces the number of games on the platform considerably.
                        However, some Windows titles can work on Mac OS through Wine emulator, and the others can be ported to Linux and played on Mac.
                        The system has the support of every significant game-distribution services such as Steam, Battle.net, and Origin.
                        However, not every title there is accessible.
                    </h1>
                </div>
...
const mapDispatchToProps = dispatch => ({
    getData: games => dispatch(macOSAction(games))
})

const mapStateToProps = state => ({
    gamesMacOS: state.gamesMacOS.gamesMacOS,
    isMacOSLoaded: state.gamesMacOS.isMacOSLoaded
})

export default connect(mapStateToProps, mapDispatchToProps)(PlatformMacOS);